...
export const ResumeAnalyzerPage = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-10">
      {/* Top Header Banner */}
      <div className="relative rounded-3xl bg-gradient-to-r from-brand-50 via-indigo-50/40 to-slate-50 dark:from-slate-900 dark:via-brand-950/40 dark:to-slate-900 border border-brand-200 dark:border-slate-800/80 p-8 sm:p-10 overflow-hidden shadow-sm transition-colors">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-3 max-w-2xl">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-brand-500/10 border border-brand-500/20 text-brand-600 dark:text-brand-400 text-xs font-semibold tracking-wide uppercase">
          ...
          <div className="flex items-center gap-3">
            <Link
              to="/resume"
              className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 border border-slate-300 dark:border-slate-700 text-sm font-medium transition shadow-sm"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className="bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800/80 rounded-2xl p-6 shadow-sm space-y-6 transition-colors">
            ...
            <form onSubmit={handleAnalyze} className="space-y-5">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full bg-white dark:bg-slate-950 border border-slate-300 dark:border-slate-800 rounded-xl px-4 py-2.5 text-sm text-slate-900 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-brand-500/50 focus:border-brand-500 transition"
                >
              ...
              {/* Source Content Area */}
              {activeTab === 'profile' ? (
                <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800/80 space-y-2 text-xs text-slate-600 dark:text-slate-400">
                  ...
                  <Link
                    to="/profile"
                    className="inline-flex items-center gap-1 text-brand-600 dark:text-brand-400 hover:text-brand-700 dark:hover:text-brand-300 font-medium pt-1"
                  >
                ...
                <div className="space-y-2">
                  ...
                  <textarea
                    ...
                    className="w-full bg-white dark:bg-slate-950 border border-slate-300 dark:border-slate-800 rounded-xl p-3.5 text-xs text-slate-900 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-brand-500/50 focus:border-brand-500 font-mono leading-relaxed"
                  />
                </div>
              )}
          ...
          {analyses.length > 0 && (
            <div className="bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 space-y-4 shadow-sm transition-colors">
                  ...
                  <button
                    ...
                    className={`w-full text-left p-3 rounded-xl border transition flex items-center justify-between gap-3 ${
                      currentAnalysis?._id === item._id
                        ...
                        : 'bg-slate-50 dark:bg-slate-950/40 border-slate-200 dark:border-slate-800/80 hover:bg-slate-100 dark:hover:bg-slate-800/50'
                    }`}
                  >
        ...
        <div className="lg:col-span-7 space-y-6">
          {!currentAnalysis ? (
            <div className="bg-white dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800/80 rounded-3xl p-12 text-center space-y-4 shadow-sm">
              <div className="w-16 h-16 rounded-2xl bg-brand-500/10 text-brand-600 dark:text-brand-400 mx-auto flex items-center justify-center">
                <Sparkles className="w-8 h-8" />
              </div>
              <h3 className="text-xl font-bold text-slate-900 dark:text-white">No Analysis Selected</h3>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="p-5 rounded-2xl bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col justify-between transition-colors">
              ...
              <div className="p-6 rounded-2xl bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 shadow-sm transition-colors">
                ...
                <div className="h-44 w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                      <XAxis type="number" domain={[0, 100]} stroke={chartTheme.axisStroke} tick={{ fill: chartTheme.tickFill, fontSize: 11 }} />
                      <YAxis
                        type="category"
                        dataKey="name"
                      ...
                      <Bar dataKey="value" radius={[0, 6, 6, 0]} barSize={16}>
                        {chartData.map((entry, index) => (
                          <Cell
                            key={`cell-${index}`}
                            fill={index === 0 ? '#0284c7' : index === 1 ? '#6366f1' : '#10b981'}
                          />
                        ))}
                      </Bar>
                    </BarChart>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {/* Matched Skills */}
                <div className="p-5 rounded-2xl bg-white dark:bg-slate-900/80 border border-emerald-500/20 shadow-sm space-y-3 transition-colors">
                ...
                {/* Missing Skills */}
                <div className="p-5 rounded-2xl bg-white dark:bg-slate-900/80 border border-rose-500/20 shadow-sm space-y-3 transition-colors">
              ...
              <div className="p-6 rounded-2xl bg-gradient-to-br from-brand-50/60 via-white to-slate-50 dark:from-slate-900 dark:via-brand-950/20 dark:to-slate-900 border border-brand-500/20 shadow-sm space-y-4 transition-colors">
                ...
                <div className="space-y-2.5">
                  {currentAnalysis.recommendations?.map((rec, idx) => (
                    <div
                      key={idx}
                      className="p-3 rounded-xl bg-white dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800/80 text-xs text-slate-800 dark:text-slate-200 flex items-start gap-3 shadow-xs"
                    >
                      <span className="w-5 h-5 rounded-full bg-brand-500/20 text-brand-600 dark:text-brand-400 flex items-center justify-center shrink-0 text-[11px] font-bold">
                        {idx + 1}
                      </span>
              ...
              {currentAnalysis.keywordOptimization?.length > 0 && (
                <div className="p-6 rounded-2xl bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 shadow-sm space-y-3 transition-colors">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wider">
                      Recommended Keyword Injections
                    </span>
                    <span className="text-xs text-slate-400 dark:text-slate-500">Click to copy</span>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {currentAnalysis.keywordOptimization.map((kw, idx) => (
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-100 dark:bg-slate-800/80 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 border border-slate-200 dark:border-slate-700 text-xs font-medium transition active:scale-95 shadow-xs"
                      >
                        {copiedKeyword === kw ? (
