  ...
  if (loading) {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center gap-3">
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400">Loading recruitment metrics and hiring pipeline...</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8 animate-fadeIn">
      ...
      <div className="relative overflow-hidden p-6 sm:p-8 rounded-3xl bg-white border border-slate-200 dark:bg-gradient-to-r dark:from-slate-900 dark:via-brand-950/40 dark:to-slate-900 dark:border-slate-800 shadow-sm dark:shadow-xl">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 relative z-10">
          ...
          <div className="flex flex-wrap items-center gap-3 shrink-0">
            <Link
              to="/recruiter/jobs"
              className="inline-flex items-center gap-1.5 px-4 py-2.5 rounded-xl bg-gradient-to-r from-brand-600 to-cyan-600 hover:from-brand-500 hover:to-cyan-500 text-white text-xs font-bold shadow-md shadow-brand-500/20 transition"
            >
            ...
            <Link
              ...
              className="inline-flex items-center gap-1.5 px-4 py-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-800 border border-slate-300 dark:bg-slate-850 dark:hover:bg-slate-800 dark:text-slate-200 dark:border-slate-750 text-xs font-semibold transition"
            >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4">
        {/* KPI 1: Total Jobs */}
        <div className="p-4 sm:p-5 rounded-2xl bg-white border border-slate-200 dark:bg-slate-900/80 dark:border-slate-800 shadow-sm space-y-1">
          <div className="flex items-center justify-between text-slate-500 dark:text-slate-400">
            <span className="text-xs font-semibold">Total Jobs</span>
            <Briefcase className="w-4 h-4 text-slate-400" />
          </div>
          <p className="text-2xl font-black text-slate-900 dark:text-white">{kpis.totalJobs}</p>
          <p className="text-[11px] text-slate-500">{kpis.closedJobs} closed • {kpis.draftJobs} draft</p>
        </div>

        {/* KPI 2: Active Jobs */}
        <div className="p-4 sm:p-5 rounded-2xl bg-white border border-emerald-200 dark:bg-slate-900/80 dark:border-emerald-500/20 shadow-sm space-y-1">
          <div className="flex items-center justify-between text-emerald-600 dark:text-emerald-400">
            <span className="text-xs font-semibold">Active Jobs</span>
            <CheckCircle2 className="w-4 h-4" />
          </div>
          <p className="text-2xl font-black text-emerald-600 dark:text-emerald-300">{kpis.activeJobs}</p>
        ...
        <Link
          to="/recruiter/applicants"
          className="p-4 sm:p-5 rounded-2xl bg-white border border-slate-200 dark:bg-slate-900/80 dark:border-slate-800 hover:border-brand-500/40 shadow-sm transition group space-y-1"
        >
          <div className="flex items-center justify-between text-slate-500 dark:text-slate-400">
            <span className="text-xs font-semibold">Total Applicants</span>
            <Users className="w-4 h-4 text-brand-600 dark:text-brand-400 group-hover:scale-110 transition" />
          </div>
          <p className="text-2xl font-black text-slate-900 dark:text-white">{kpis.totalApplicants}</p>
        ...
        <Link
          to="/recruiter/applicants"
          className="p-4 sm:p-5 rounded-2xl bg-white border border-purple-200 dark:bg-slate-900/80 dark:border-purple-500/20 shadow-sm transition space-y-1"
        >
          <div className="flex items-center justify-between text-purple-600 dark:text-purple-400">
            <span className="text-xs font-semibold">Shortlisted</span>
            <UserCheck className="w-4 h-4" />
          </div>
          <p className="text-2xl font-black text-purple-600 dark:text-purple-300">{kpis.shortlisted}</p>
        ...
        <Link
          to="/recruiter/interviews"
          className="p-4 sm:p-5 rounded-2xl bg-white border border-cyan-200 dark:bg-slate-900/80 dark:border-cyan-500/20 shadow-sm transition space-y-1"
        >
          ...
          <p className="text-2xl font-black text-cyan-600 dark:text-cyan-300">{kpis.interviews}</p>
          <p className="text-[11px] text-slate-500">{upcomingInterviews.length} active sessions</p>
        </Link>

        {/* KPI 6: Selected / Hired */}
        <div className="p-4 sm:p-5 rounded-2xl bg-white border border-amber-200 dark:bg-slate-900/80 dark:border-amber-500/20 shadow-sm space-y-1">
          <div className="flex items-center justify-between text-amber-600 dark:text-amber-400">
            <span className="text-xs font-semibold">Selected</span>
            <Award className="w-4 h-4" />
          </div>
          <p className="text-2xl font-black text-amber-600 dark:text-amber-300">{kpis.selected}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 p-6 sm:p-8 rounded-3xl bg-white border border-slate-200 dark:bg-slate-900/80 dark:border-slate-800 shadow-sm space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-200 dark:border-slate-800 pb-4">
            ...
            <div className="flex items-center gap-2">
              <span className="px-3 py-1 rounded-xl bg-purple-50 text-purple-700 border border-purple-200 dark:bg-purple-500/10 dark:text-purple-300 dark:border-purple-500/30 text-xs font-bold">
                {kpis.shortlistRate}% Shortlisted
              </span>
              <span className="px-3 py-1 rounded-xl bg-emerald-50 text-emerald-700 border border-emerald-200 dark:bg-emerald-500/10 dark:text-emerald-300 dark:border-emerald-500/30 text-xs font-bold">
                {kpis.hireRate}% Hired
              </span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-2">
            {recruitmentFunnel.map((item) => (
              <div
                key={item.stage}
                className="p-3 rounded-xl bg-slate-50 border border-slate-200 dark:bg-slate-850/60 dark:border-slate-800 text-center space-y-1"
              >
          ...
          {/* Recharts Funnel Bar Chart with dynamic theme */}
        ...
        <div className="space-y-6">
          ...
          {upcomingInterviews.length > 0 ? (
            <div className="p-6 rounded-3xl bg-white border border-cyan-200 dark:bg-gradient-to-br dark:from-slate-900 dark:via-cyan-950/20 dark:to-slate-900 dark:border-cyan-500/30 shadow-sm dark:shadow-xl space-y-4">
              ...
              <div className="space-y-1">
                <h3 className="text-base font-bold text-slate-900 dark:text-white">{upcomingInterviews[0].candidate?.name}</h3>
                <p className="text-xs text-slate-500 dark:text-slate-400">
                  Interviewing for <span className="text-cyan-600 dark:text-cyan-300 font-semibold">{upcomingInterviews[0].job?.title}</span>
                </p>
              </div>
              ...
              <div className="pt-1 flex flex-col gap-2">
                ...
                <Link
                  to="/recruiter/interviews"
                  className="w-full py-2 px-3 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-800 border border-slate-200 dark:bg-slate-850 dark:hover:bg-slate-800 dark:text-slate-300 dark:border-slate-750 text-xs font-semibold text-center transition flex items-center justify-center gap-1.5"
                >
          ...
          <div className="p-6 rounded-3xl bg-white border border-slate-200 dark:bg-slate-900/80 dark:border-slate-800 shadow-sm space-y-3">
            ...
            <div className="space-y-2.5">
              {recruiterInsights.map((item, idx) => (
                <div
                  key={idx}
                  className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200 dark:bg-slate-850/60 dark:border-slate-800 space-y-1.5"
                >
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-slate-900 dark:text-white">{item.title}</span>
                    <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${
                      item.priority === 'critical' ? 'bg-cyan-50 text-cyan-700 dark:bg-cyan-500/20 dark:text-cyan-300' :
                      item.priority === 'high' ? 'bg-amber-50 text-amber-800 dark:bg-amber-500/20 dark:text-amber-300' :
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="p-6 sm:p-8 rounded-3xl bg-white border border-slate-200 dark:bg-slate-900/80 dark:border-slate-800 shadow-sm space-y-4">
          <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-3">
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">Total applicants submitted per job</p>
            </div>
            <Link
              to="/recruiter/jobs"
          ...
          {activeVacanciesPerformance.length === 0 ? (
            <div className="p-8 text-center bg-slate-50 border border-slate-200 dark:bg-slate-950/40 dark:border-slate-800 rounded-2xl text-slate-500 text-xs space-y-2">
              ...
              <Link
                to="/recruiter/jobs"
                className="inline-block px-3.5 py-1.5 bg-brand-600 text-white rounded-xl text-xs font-semibold shadow-sm"
              >
                Create Job Vacancy
              </Link>
            </div>
            ...
            <div className="divide-y divide-slate-200 dark:divide-slate-800/60 rounded-2xl bg-white dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800/80 overflow-hidden shadow-sm">
              {activeVacanciesPerformance.map((job) => (
                <div
                  key={job._id}
                  className="p-4 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-900/40 transition gap-4"
                >
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    <span className="px-2.5 py-1 rounded-xl bg-brand-50 border border-brand-200 text-brand-700 dark:bg-brand-500/10 dark:border-brand-500/30 dark:text-brand-300 text-xs font-bold">
                      {job.applicantsCount} Applicants
                    </span>

                    <Link
                      to="/recruiter/applicants"
                      className="p-2 rounded-xl bg-slate-100 hover:bg-brand-600 hover:text-white text-slate-600 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-brand-600 dark:hover:text-white transition"
        ...
        <div className="p-6 sm:p-8 rounded-3xl bg-white border border-slate-200 dark:bg-slate-900/80 dark:border-slate-800 shadow-sm space-y-4">
          <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-3">
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">Latest candidate submissions with AI compatibility</p>
            </div>
            <Link
              to="/recruiter/applicants"
              className="text-xs font-bold text-brand-600 dark:text-brand-400 hover:underline flex items-center gap-1"
          ...
          {recentApplicants.length === 0 ? (
            ...
            <div className="divide-y divide-slate-200 dark:divide-slate-800/60 rounded-2xl bg-white dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800/80 overflow-hidden shadow-sm">
              {recentApplicants.map((app) => (
                <div
                  key={app._id}
                  className="p-3.5 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-900/40 transition gap-3"
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="w-9 h-9 rounded-xl bg-brand-50 border border-brand-200 text-brand-700 dark:bg-brand-500/10 dark:border-brand-500/20 dark:text-brand-400 font-bold text-xs flex items-center justify-center shrink-0">
                      {app.candidate?.name?.[0] || 'C'}
                    </div>
                    <div className="min-w-0">
                      <p className="text-xs font-bold text-slate-900 dark:text-white truncate">{app.candidate?.name}</p>
                      <p className="text-[11px] text-slate-500 dark:text-slate-400 truncate">{app.job?.title}</p>
                    </div>
                  </div>
