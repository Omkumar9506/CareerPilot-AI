  ...
  if (loading) {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center gap-3">
        ...
        <p className="text-sm text-slate-400">Assembling your intelligent career dashboard...</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8 animate-fadeIn">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-slate-400">• Ready for hiring</span>
            </div>
          ...
          <div className="p-4 rounded-2xl bg-slate-950/70 border border-slate-800 shrink-0 min-w-[260px] space-y-2">
            <div className="flex items-center justify-between">
              ...
              <span className={`text-xs font-black ${
                profileCompletion.score >= 80 ? 'text-emerald-400' : 'text-amber-400'
              }`}>
            ...
            <div className="flex items-center justify-between pt-1">
              ...
              <Link
                to="/profile"
                className="text-[11px] font-bold text-brand-400 hover:text-brand-300 flex items-center gap-0.5"
              >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Link
          to="/applications"
          className="p-5 rounded-2xl bg-slate-900/80 border border-slate-800 hover:border-brand-500/40 shadow transition group space-y-1"
        >
          <div className="flex items-center justify-between text-slate-400">
            <span className="text-xs font-semibold">Applications</span>
            <div className="w-8 h-8 rounded-xl bg-brand-500/10 text-brand-400 flex items-center justify-center group-hover:scale-110 transition">
              <FileText className="w-4 h-4" />
            </div>
          </div>
          <p className="text-2xl font-black text-white">{appStats.total || 0}</p>
          <p className="text-[11px] text-slate-500">
            {appStats.interview > 0 ? (
              <span className="text-cyan-400 font-medium">⚡ {appStats.interview} in interview stage</span>
            ) : (
              'Active submissions'
            )}
          </p>
        ...
        <Link
          to="/interviews"
          className="p-5 rounded-2xl bg-slate-900/80 border border-slate-800 hover:border-cyan-500/40 shadow transition group space-y-1"
        >
          <div className="flex items-center justify-between text-slate-400">
            <span className="text-xs font-semibold">Upcoming Interviews</span>
            <div className="w-8 h-8 rounded-xl bg-cyan-500/10 text-cyan-400 flex items-center justify-center group-hover:scale-110 transition">
              <Calendar className="w-4 h-4" />
            </div>
          </div>
          <p className="text-2xl font-black text-white">{upcomingInterviews.length}</p>
          <p className="text-[11px] text-slate-500">
            {upcomingInterviews.length > 0 ? (
              <span className="text-emerald-400 font-semibold">Next call scheduled</span>
            ) : (
        ...
        <div
          ...
          className="p-5 rounded-2xl bg-slate-900/80 border border-slate-800 hover:border-amber-500/40 shadow transition group space-y-1 cursor-pointer"
        >
          <div className="flex items-center justify-between text-slate-400">
          ...
          <p className="text-2xl font-black text-white">{savedJobsCount || 0}</p>
        ...
        <Link
          to="/resume-analyzer"
          className="p-5 rounded-2xl bg-slate-900/80 border border-slate-800 hover:border-purple-500/40 shadow transition group space-y-1"
        >
          <div className="flex items-center justify-between text-slate-400">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="p-6 rounded-3xl bg-slate-900/80 border border-slate-800 shadow space-y-6">
            <div className="flex items-center justify-between border-b border-slate-800 pb-4">
              ...
              {/* Sub tabs */}
              <div className="flex items-center gap-1 bg-slate-850 p-1 rounded-xl border border-slate-800 text-xs font-semibold">
                <button
                  onClick={() => setActiveTab('pipeline')}
                  className={`px-3 py-1.5 rounded-lg transition ${
                    activeTab === 'pipeline' ? 'bg-brand-600 text-white' : 'text-slate-400 hover:text-white'
                  }`}
                >
                  Pipeline Stages
                </button>
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-2">
                  {pipelineFunnel.map((item) => (
                    <div
                      key={item.stage}
                      className="p-3 rounded-xl bg-slate-850/70 border border-slate-800 text-center space-y-1"
                    >
                ...
                <div className="space-y-3 pt-2">
                  <div className="flex items-center justify-between text-xs font-semibold text-slate-400">
                    ...
                    <Link to="/applications" className="text-brand-400 hover:underline flex items-center gap-1">
                  ...
                  {recentApplications.length === 0 ? (
                    <div className="p-8 text-center bg-slate-950/40 rounded-2xl border border-slate-800 text-slate-500 text-xs space-y-2">
                      ...
                      <Link
                        to="/jobs"
                        className="inline-block px-3 py-1.5 bg-brand-600/20 text-brand-400 border border-brand-500/30 rounded-lg text-xs font-bold"
                      >
                    ...
                    <div className="divide-y divide-slate-800/60 rounded-2xl bg-slate-950/40 border border-slate-800/80 overflow-hidden">
                      {recentApplications.map((app) => (
                        <div key={app._id} className="p-3.5 flex items-center justify-between hover:bg-slate-900/40 transition">
                          <div className="space-y-0.5">
                            <p className="text-xs font-bold text-white">{app.job?.title || 'Job Listing'}</p>
                ...
                {recentSavedJobs.length === 0 ? (
                  ...
                  <div className="divide-y divide-slate-800/60 rounded-2xl bg-slate-950/40 border border-slate-800/80 overflow-hidden">
                    {recentSavedJobs.map((item) => (
                      <div key={item._id} className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 hover:bg-slate-900/40 transition">
                        <div className="space-y-1">
                          <p className="text-sm font-bold text-white">{item.job?.title}</p>
                          <p className="text-xs text-slate-400">
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          <Link
                            ...
                            className="px-3 py-1.5 rounded-lg bg-brand-600 text-white text-xs font-bold hover:bg-brand-500 transition"
                          >
                            Apply Now
                          </Link>
                          <button
                            onClick={() => handleUnsaveJob(item.job?._id)}
                            className="p-1.5 rounded-lg bg-slate-800 text-slate-400 hover:text-rose-400 hover:bg-slate-750 transition"
        ...
        <div className="space-y-6">
          ...
          {upcomingInterviews.length > 0 ? (
            <div className="p-6 rounded-3xl bg-gradient-to-br from-slate-900 via-cyan-950/20 to-slate-900 border border-cyan-500/30 shadow-xl space-y-4">
              ...
              <div className="space-y-1">
                <h3 className="text-base font-bold text-white">{upcomingInterviews[0].job?.title}</h3>
          ...
          <div className="p-6 rounded-3xl bg-slate-900/80 border border-slate-800 shadow space-y-4">
            <div className="flex items-center justify-between">
            ...
            <div className="space-y-3">
              {aiInsights.map((insight, idx) => (
                <div
                  key={idx}
                  className="p-3.5 rounded-2xl bg-slate-850/60 border border-slate-800 hover:border-slate-700 transition space-y-2"
                >
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-white">{insight.title}</span>
                    <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${
                      insight.priority === 'critical' ? 'bg-cyan-500/20 text-cyan-300' :
                      insight.priority === 'high' ? 'bg-amber-500/20 text-amber-300' :
      ...
      <div className="p-6 sm:p-8 rounded-3xl bg-slate-900/80 border border-slate-800 shadow space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <Link
            to="/jobs"
            className="inline-flex items-center gap-1 text-xs font-bold text-brand-400 hover:text-brand-300 self-start sm:self-auto"
          >
        ...
        {recommendedJobs.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {recommendedJobs.map((job) => (
              <div
                key={job._id}
                className="p-5 rounded-2xl bg-slate-850/60 border border-slate-800 hover:border-brand-500/40 transition flex flex-col justify-between space-y-4 group"
              >
                <div className="space-y-2">
                  ...
                  <h3 className="text-sm font-bold text-white group-hover:text-brand-300 transition line-clamp-1">
                    {job.title}
                  </h3>
                ...
                <Link
                  to={`/jobs/${job._id}`}
                  className="w-full py-2 rounded-xl bg-slate-800 hover:bg-brand-600 text-white text-xs font-bold text-center transition flex items-center justify-center gap-1 group-hover:bg-brand-600"
                >
